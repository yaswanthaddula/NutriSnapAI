...
export default function TuesdayDetail() {
  const { isDark } = useTheme();
  const params = useLocalSearchParams();
  const { addNotification, completeHealthDay, waterData } = useAppStore();
  
  const dayName = 'Tuesday';
  const planTitle = 'Low Sugar & Hydration';
  const calorieBurn = 300;

  const [tasks, setTasks] = useState([
    { id: 1, text: 'Avoid sugary drinks', icon: 'food-off', iconType: 'MCI', color: '#FFEBEE', completed: false },
    { id: 2, text: 'Drink 3L water', icon: 'water', iconType: 'Ionicons', color: '#E3F2FD', completed: false },
    { id: 3, text: 'Eat balanced meals', icon: 'food-apple', iconType: 'MCI', color: '#E8F5E9', completed: false },
    { id: 4, text: '15 min Guided Meditation', icon: 'meditation', iconType: 'MCI', color: '#FFF3E0', completed: false },
    { id: 5, text: 'No Caffeine after 4 PM', icon: 'coffee-off', iconType: 'MCI', color: '#FFF9C4', completed: false },
  ]);

  // Sync water task
  useEffect(() => {
    if (waterData && waterData.today >= 3000) {
  ...
  const handleComplete = () => {
    if (completedCount === tasks.length) {
      ...
      addNotification({
        title: "Day Complete!",
        message: `You've completed all Tuesday health goals!`,
        type: 'success',
        mode: 'health'
      });
      Alert.alert("Namaste!", `You finished all your goals for ${dayName}!`, [
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={[styles.interactiveCard, { backgroundColor: theme.card, borderColor: theme.border }]}>
          ...
          <View style={styles.activityRow}>
            <View style={styles.activityIcon}><Ionicons name="water" size={24} color="#2196F3" /></View>
            <View style={{ flex: 1, marginLeft: 15 }}>
              <Text style={[styles.activityLabel, { color: theme.text }]}>Water Intake</Text>
              <Text style={[styles.activityVal, { color: theme.subText }]}>{waterData?.today || 0} / 3,000 ml</Text>
              <View style={[styles.miniProgress, { backgroundColor: isDark ? '#333' : '#E0E0E0' }]}><View style={[styles.miniBar, { width: `${Math.min(100, ((waterData?.today || 0)/3000)*100)}%`, backgroundColor: '#2196F3' }]} /></View>
            </View>
          </View>

          {/* Checklist Tip */}
          <View style={[styles.activityRow, { marginTop: 20 }]}>
            <View style={styles.activityIcon}><MaterialCommunityIcons name="food-off" size={24} color="#FF5252" /></View>
            <View style={{ flex: 1, marginLeft: 15 }}>
              <Text style={[styles.activityLabel, { color: theme.text }]}>Sugar-Free Challenge</Text>
              <Text style={[styles.activityVal, { color: theme.subText }]}>Replace soda with water or tea.</Text>
            </View>
          </View>
        </View>
        ...
        <View style={styles.titleSection}>
          ...
          <Text style={[styles.subTitle, { color: theme.subText }]}>Focus: Mental Clarity & Flexibility</Text>
        </View>
