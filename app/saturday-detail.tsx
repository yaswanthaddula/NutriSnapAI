...
export default function SaturdayDetail() {
  const { isDark } = useTheme();
  const params = useLocalSearchParams();
  const { addNotification, completeHealthDay } = useAppStore();
  
  const dayName = 'Saturday';
  const planTitle = 'Outdoor Adventure';
  const calorieBurn = 500;
  const videoId = params.videoId as string || 'ml6cT4AZdqI';

  const [tasks, setTasks] = useState([
    { id: 1, text: '1 hour Cycling or Outdoor activity', icon: 'bike', iconType: 'MCI', color: '#E8F5E9', completed: false },
    { id: 2, text: 'Eat a large fruit bowl', icon: 'fruit-watermelon', iconType: 'MCI', color: '#FFF3E0', completed: false },
    { id: 3, text: 'Drink 3 liters water', icon: 'water', iconType: 'Ionicons', color: '#E3F2FD', completed: false },
    { id: 4, text: 'Clean and organize space', icon: 'broom', iconType: 'MCI', color: '#FFEBEE', completed: false },
    { id: 5, text: 'Take a relaxing bath', icon: 'bathtub-outline', iconType: 'MCI', color: '#FFF9C4', completed: false },
  ]);
  ...
  const handleComplete = () => {
    if (completedCount === tasks.length) {
      ...
      addNotification({
        title: "Day Complete!",
        message: `You've completed all Saturday health goals!`,
        type: 'success'
      });
      Alert.alert("Great Day!", `You finished all your goals for ${dayName}!`, [
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: theme.background }]}>
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.titleSection}>
          ...
          <Text style={[styles.subTitle, { color: theme.subText }]}>Focus: Nature & Active Living</Text>
        </View>
